...
csv_writer.writerow(["iteration", "running loss", "val loss", "val loss (mp)", "train acc", "train acc (mp)", "val acc", "val acc (mp)", "time", "max memory", "f-nfe", "b-nfe"])
    ...
    for itr in range(args.nepochs * batches_per_epoch):
        ...
        if itr % batches_per_epoch == 0:
            with torch.no_grad():
                ...
                with autocast(device_type='cuda', dtype=args.precision):
                    train_acc_mp = accuracy(model, train_eval_loader)
                    val_logits_mp = model(x)
                    val_loss_mp = criterion(val_logits_mp, y)
                    val_acc_mp = accuracy(model, test_loader)
                

                if val_acc > best_acc:
                    torch.save({'state_dict': model.state_dict(), 'args': args},  os.path.join(result_dir, folder_name + "model.pth"))
                    best_acc = val_acc
                
                print('Iter {:04d} | Running Loss {:.6f} | Val Loss  {:.6f} | Val Loss (mp) {:.6f} | Train Acc {:.4f} | Train Acc (mp) {:.4f} | Val Acc {:.4f} | Val Acc (mp) {:.4f} |  Time {:.4f}s | Max Memory {:.1f}MB | F-NFE {:1.2f} | B-NFE {:1.2f}'.format(
                    itr, loss_meter.avg, val_loss.item(), val_loss_mp.item(), train_acc, train_acc_mp, val_acc, val_acc_mp, time_meter.avg, peak_memory, f_nfe_meter.avg, b_nfe_meter.avg))
                
                csv_writer.writerow([itr, loss_meter.avg, val_loss.item(), val_loss_mp.item(), train_acc, train_acc_mp, val_acc, val_acc_mp, time_meter.avg, peak_memory, f_nfe_meter.avg, b_nfe_meter.avg])
