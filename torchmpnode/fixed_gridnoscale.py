...
class FixedGridODESolver(torch.autograd.Function):
    ...
    def forward(ctx, step, func, y0, t, *params):
        ...
        with torch.no_grad():
            for i in range(N - 1):
                ...
                # print(f"[Forward Step {i}] yt[i + 1].dtype stored at = {yt[i + 1].dtype}")
    ...
    def backward(ctx, at):
        ...
        grad_t = torch.zeros_like(t) if t.requires_grad else None
        
        with autocast(device_type='cuda', enabled=False):

            old_params = {name: param.clone() for name, param in func.named_parameters()}
            ...
            for i in reversed(range(N - 1)):
                ...
                with torch.enable_grad():
                    ...
                    # print(f"[Backward Step {i}] Retrieved y.dtype from yt = {y.dtype}") #yt = torch.float16
                    if t.requires_grad:
                        ...
                        # print(f"[Backward Step {i}] Raw da.dtype = {da.dtype}") #Raw da.dtype = torch.float16
                        gti, gdti = None, None
                        gdti2 = torch.sum(a * dy, dim=-1)
                

                a = a + dt*(da.to(dtype_hi) + at[i].to(dtype_hi))
                # print(f"[Backward Step {i}] da_hi.dtype and at[i]_hi.dtype = {dtype_hi}, Updated a.dtype = {a.dtype}") #da_hi.dtype and at[i]_hi.dtype = torch.float32, Updated a.dtype = torch.float32
