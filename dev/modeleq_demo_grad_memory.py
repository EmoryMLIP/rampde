...
# with torch.autograd.detect_anomaly():
    ...
    # Calculate total memory allocated for gradients
    total_memory = grad_x_num.element_size() * grad_x_num.nelement() + grad_A_num.element_size() * grad_A_num.nelement()
    # print(f"Total memory allocated for gradients: {total_memory} bytes")
    # print(grad_A_num.dtype)
    ...
    return rel_error_solution.item(), rel_error_grad_x.item(), rel_error_grad_A.item(), total_memory, grad_x_num.dtype, grad_A_num.dtype


# --- Main experiment ---
if __name__ == '__main__':
    # We will test both single precision and double precision.
    dtypes_f = [ torch.float16]
    dtypes_y = [ torch.float16, torch.float32]
                ...
                for package in packages:
                    ...
                    if method in ['euler', 'rk4']:
                        ...
                        for n_steps in steps_list:
                            sol_err, grad_x_err, grad_A_err, grad_memory, grad_x_dtype, grad_A_dtype = run_experiment(ode,method, dtype_f,dtype_y, n_steps=n_steps)
                            print(f"n_steps = {n_steps:3d}  |  "
                                f"sol. relerr: {sol_err:8.2e}  |  "
                                f"∂y/∂x relerr: {grad_x_err:8.2e}  |  "
                                f"∂y/∂A relerr: {grad_A_err:8.2e}  |  "
                                f"grad memory: {grad_memory:8.2e}  |  "
                                f"grad x dtype: {grad_x_dtype}  |  "
                                f"grad A dtype: {grad_A_dtype}".replace("torch.", ""))
                            results.append({
                                "dtype_f": str(dtype_f),
                                "dtype_y": str(dtype_y),
                                "method": method,
                                ...
                                "grad_x_dtype": str(grad_x_dtype),
                                "grad_A_dtype": str(grad_A_dtype)
                            })
                        print()
    import pandas as pd
    df = pd.DataFrame(results)
    df.to_csv("modeleq_demo_grad_memory.csv", index=False)
    print("\nResults saved to modeleq_demo_grad_memory.csv")
