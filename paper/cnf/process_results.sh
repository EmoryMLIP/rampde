...
SEED=${1:-24}  # Default to seed 24 (test runs)
...
# Also extract target images (from float32_torchdiffeq)
for dataset in "${datasets[@]}"; do
    pattern="raw_data/cnf/${dataset}_float32_torchdiffeq_rk4_*_seed${SEED}_*"
    exp_dir=$(ls -dt $pattern 2>/dev/null | head -1)
