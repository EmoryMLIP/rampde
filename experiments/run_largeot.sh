  ...
  for precision in "${precisions[@]}"; do
    for method in "${methods[@]}"; do
      for odeint in "${odeints[@]}"; do
        ...
        # log file name
        logf="logs/${dataset}_${precision}_${method}_${odeint}.log"

        echo "↪ Running: data=$dataset precision=$precision method=$method odeint=$odeint"
        echo "  -> logging to $logf"

        # run & capture both stdout and stderr
        python otflowlarge.py "${fixed_args[@]}" $extra_args \
          &> "$logf"

        # check exit status
        if [ $? -ne 0 ]; then
          echo "Run failed! See $logf for details."
        else
